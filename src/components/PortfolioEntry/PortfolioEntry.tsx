...
const PortfolioEntry: React.FC<PortfolioEntryProps> = ({ entry, onDisplayEditModal: onDisplayModal, loggedIn }) => {
  return (
      <div className="entry-container flex border-1 border-solid border-black rounded-3xl p-2.5 text-center m-4 bg-[#EAEAEA] drop-shadow-md " key={entry.id}>
        ...
        {loggedIn && <Button className= {"flex justify-end"} onClick={() => onDisplayModal(entry.id)} buttonText="Edit"/>}
